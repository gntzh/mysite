...
def getDesMethod(user):
    ...
    iv = key
    method = pyDes.des(key, mode=pyDes.CBC, IV=iv,
...
def generateVerifyEmailUrl(user):
    if isinstance(user, User):
        try:
            data = bytes(datetime.now().strftime('%Y-%m-%d %H:%M:%S') +
                         '@' + str(user.id) + '@' + user.username, encoding='utf-8')
            method = getDesMethod(user)
            token = binascii.b2a_hex(
                method.encrypt(data, padmode=pyDes.PAD_PKCS5))
            user_id = bytes(str(user.id), encoding='utf-8')
            key = base64.b64encode(b'%s@%s' % (
                token, user_id,)).decode('utf-8')
            verify_url = '%s/api-user/verify_email?key=%s' % (
                settings.SITE_HOST, key)
            return verify_url
        except:
            return False
...
def checkVerifyEmailUrl(key):
    token, user_id = base64.b64decode(
        key.encode('utf-8')).decode('utf-8').split('@')
    user_id = int(user_id)
    user = User.objects.get(id=user_id)
    method = getDesMethod(user)
    data = method.decrypt(binascii.a2b_hex(token.encode(
        'utf-8')), padmode=pyDes.PAD_PKCS5).decode('utf-8')
    print(data)
    now = datetime.now()
